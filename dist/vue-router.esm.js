...
var View = {
  ...
  render: function render (_, ref) {
    ...
    // 用于vue开发者工具devtools 显示有router-view
    data.routerView = true;

    // 使用父级的context对象上的 createElement 函数进行组件渲染
...
function createMatcher (
  ...
  function match (
    ...
    // 如果有路由名称 就进行nameMap映射 获取到路由记录 处理路由params 返回一个_createRoute处理的东西
    if (name) {
...
var HashHistory = /*@__PURE__*/(function (History) {
  ...
  HashHistory.prototype.setupListeners = function setupListeners () {
    ...
    // 通过事件监听来调用transitionTo，实现路由和视图统一
...
// url同步 如果支持pushState用pushState，否则用window.location.hash
...
var VueRouter = function VueRouter (options) {
  ...
  // 不同的mode，实例化不同的History类, 后面的history就是History的实例
  switch (mode) {
    case 'history':
...
// this.$router.match对外暴露 可以通过传目标路径和当前的route对象，返回要跳的路由对象
...
VueRouter.prototype.init = function init (app /* Vue component instance */) {
  ...
  // $once 监听自定义事件，但是只触发一次，一旦触发后，监听器就会被移除
  ...
  // 针对不同模式做不同的处理 transitionTo是history的核心方法
